...
@user_router.get("/get")
async def get_user(
    login: Optional[str] = None, auth_user: User = Depends(get_auth_active_user)
) -> User | List[User]:
    if login:
...
async def add_user(
    user: UserInput, auth_user: User = Depends(get_auth_active_user)
) -> None:
...
async def delete_user(
    login: str, auth_user: User = Depends(get_auth_active_user)
) -> None:
    await user_service.user_store.delete(login)


opportunity_router = APIRouter(prefix="/opportunity", tags=tags)


@opportunity_router.get("/get")
async def get_opportunity(
    id: Optional[str] = None, auth_user: User = Depends(get_auth_active_user)
...
role_router = APIRouter(prefix="/role", tags=tags)
...
async def add_role(
    role: RoleInput, auth_user: User = Depends(get_auth_active_user)
) -> IdSchema:
